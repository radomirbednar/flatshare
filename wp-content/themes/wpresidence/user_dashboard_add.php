...
if (isset($_GET['listing_edit']) && is_numeric($_GET['listing_edit'])) {
    ...
    $how_long = esc_html(get_post_meta($edit_id, 'how_long', true));
    $sexual_preference = esc_html(get_post_meta($edit_id, 'sexual_preference', true));
    $user_gender = esc_html(get_post_meta($edit_id, 'user_gender', true));
    $sleeping_span = esc_html(get_post_meta($edit_id, 'sleeping_span', true));
    $couple = esc_html(get_post_meta($edit_id, 'couple', true));
    $pets = esc_html(get_post_meta($edit_id, 'pets', true));
    $smoker = esc_html(get_post_meta($edit_id, 'smoker', true));
    $party = esc_html(get_post_meta($edit_id, 'party', true));
...
if ('POST' == $_SERVER['REQUEST_METHOD'] && $_POST['action'] == 'view') {
    ...
    if ($paid_submission_status != 'membership' || ( $paid_submission_status == 'membership' || wpestate_get_current_user_listings($userID) > 0)) { // if user can submit
        ...
        $property_label_before = wp_kses($_POST['property_label_before'], $allowed_html);
        $property_size = wp_kses($_POST['property_size'], $allowed_html);
        $owner_notes = wp_kses($_POST['owner_notes'], $allowed_html);
        ...
        $google_view = wp_kses($_POST['property_google_view'], $allowed_html);
        ...
        if ($post_id) {
            ...
            if (!isset($_POST['sexual_preference'])) {
                $sexual_preference = 3;
            } else {
            ...
            update_post_meta($post_id, 'sleeping_span', $sleeping_span);
            update_post_meta($post_id, 'couple', $couple);
            update_post_meta($post_id, 'pets', $pets);
            update_post_meta($post_id, 'smoker', $smoker);
...
if ('POST' == $_SERVER['REQUEST_METHOD'] && $_POST['action'] == 'edit') {
    ...
    /*
      ...
      $party = (int) ( $_POST['party']);
      $rent_amount = (int) ( $_POST['rent_amount'] );
      $user_origin = (int) ( $_POST['user_origin']);
      $activity = (int) ($_POST['activity']);
      $language = ($_POST['language']);
     */
