...
function fl_get_user_language_ids($user_id){    
    global $wpdb;
    $sql = "SELECT lu.id_lang FROM fl_language_user AS lu JOIN fl_language2user AS l2u ON lu.id_lang = li.id_lang WHERE l2u.id_user = '" . (int) $user_id . "'";
...
function fl_get_user_house_skills_ids(){
    global $wpdb;
    $sql = "SELECT id_skill FROM fl_skill_user AS su JOIN fl_skill2user AS s2u ON su.id_skill = s2u.id_skill WHERE id_user = '" . (int) $id_user . "'";
    $result = $wpdb->get_results($sql);
    return $result;
}
